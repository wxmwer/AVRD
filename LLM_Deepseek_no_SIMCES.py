...
class SimcseModel(nn.Module):
    ...
    def __init__(self, pretrained_model, pooling):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(pretrained_model, config=config, torch_dtype=torch.bfloat16)
    ...
    if Consider_time:
        ...
        for i in range(len(da_qu)):
            ...
            if Consider_matching_criteria:
                ...
                st_te_match = te_time_match[
                    te_time_match['co_is_match'] & (~te_time_match['教工号加密'].isin(st_te_pool_reco))]
...
if __name__ == '__main__':
    ...
    BERT_AVRD = base_url + 'pretrained_model/deepseek-llm-7b-chat'
    model_path = BERT_AVRD

    # Load pretrained model/tokenizer
    # tok无影响
    tokenizer = AutoTokenizer.from_pretrained(model_path)  # model_path#'pretrained_model/bert_wwm_ext_chinese_pytorch'
    ...
    Rate_list = [0.3, 0.5, 0.6, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0]  # 0.9, 0.92, 0.95, 1
        ...
        for POOLING in ['cls']:
            ...
            for Parameter_2 in Rate_list:
                ...
                reco_ID_all_Model, reco_true_model = Model_run_time(model_name,
                                                                    te_co_=te_co2, te_di_=te_di2, te_pa_=te_pa2,
                                                                    da_qu_=da_qu_1,
                                                                    Rate_=Rate_, N_=15, Pool_reco_size_=13,
                                                                    Match_reco_size_=2,
                                                                    Consider_time_=True, match_crit=False,
                                                                    trut_Name_all_=st_te_truth_all_name,
                                                                    ID_Name_=ID_NAME)
                print(POOLING)
                print('\n', reco_ID_all_Model)
