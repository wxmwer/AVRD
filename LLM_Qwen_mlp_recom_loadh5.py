...
class SimcseModel(nn.Module):
    ...
    def __init__(self, d_model=3584, dropout=0.1):
        ...
        self.linear2 = nn.Linear(d_model // 2, 1024)
        self.activation = nn.GELU()
        self.linear3 = nn.Linear(1024, D_MODEL)
...
if __name__ == '__main__':
    ...
    for i in range(len(Save_Path)):
        ...
        print("SimcseModel load successfully")
        ...
        for i in range(len(da_qu_1)):
            ...
            tensor_3_out = model(tensor_3.to(DEVICE))
            tensor_2_out = model(tensor_2.to(DEVICE))
            tensor_1_out = model(tensor_1.to(DEVICE))
            da_qu_1.iat[i, -3] = tensor_3_out.squeeze().cpu().detach().numpy()
            da_qu_1.iat[i, -2] = tensor_2_out.squeeze().cpu().detach().numpy()
            da_qu_1.iat[i, -1] = tensor_1_out.squeeze().cpu().detach().numpy()
        ...
        for i in range(len(te_time_pool)):
            ...
            tensor_2_out = model(tensor_2.to(DEVICE))
            tensor_1_out = model(tensor_1.to(DEVICE))
            te_time_pool.iat[i, -3] = tensor_3_out.squeeze().cpu().detach().numpy()
            te_time_pool.iat[i, -2] = tensor_2_out.squeeze().cpu().detach().numpy()
            te_time_pool.iat[i, -1] = tensor_1_out.squeeze().cpu().detach().numpy()
        for i in range(len(te_time_match)):
            tensor_1 = torch.tensor(te_time_match.iloc[i, -1], dtype=torch.float)
            tensor_1_out = model(tensor_1.to(DEVICE))
            te_time_match.iat[i, -1] = tensor_1_out.squeeze().cpu().detach().numpy()
        ...
        reco_ID_all_Model, reco_true_model = Model_run_time(te_time_pool, te_time_match, da_qu_1,
                                                            ...
                                                            Pool_reco_size_=10, Match_reco_size_=2, match_crit=False)
        print('\n', reco_ID_all_Model)
